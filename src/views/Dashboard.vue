...
.h-full.w-full.flex.flex-col.gap-6.overflow-y-auto.pr-2
  .grid.grid-cols-4.gap-6
    ...
    .glass-card.rounded-2xl.p-6.flex.flex-col.gap-4
      ...
      .w-full.bg-white_5.h-1.rounded-full.mt-auto(class="bg-white/5")
        .bg-green-500.h-full.rounded-full(:style="{ width: (metrics.completedTasks / metrics.totalTasks * 100) + '%' }")
  ...
  .grid.grid-cols-3.gap-6.flex-1.min-h-0
    //- Operational Maturity (Projects List)
    .col-span-2.glass-card.rounded-2xl.p-6.flex.flex-col.gap-6.overflow-y-auto
      .flex.justify-between.items-center
      ...
      .flex.flex-col.gap-4
        .glass-panel.rounded-xl.p-5.flex.items-center.gap-6.hover_bg-white_5.transition-colors(
          ...
          button.w-10.h-10.rounded-full.bg-white_5.flex.items-center.justify-center.hover_bg-redAccent.transition-colors(class="bg-white/5 hover:bg-redAccent")
            ArrowRightIcon(:size="16")

    //- Global Activity Feed
    .col-span-1.glass-card.rounded-2xl.p-6.flex.flex-col.gap-6
      .flex.justify-between.items-center
